                       ...
                       const struct ofproto_async_msg *am)
{
    ...
    {
        //sprintf(str,"%x,%x,%d,%d",header.nw_src,header.nw_dst,header.tp_src,header.tcp_flags);
        fp=fopen("/home/tuyen/prj/debug.log","a");
        str=(char *)malloc(STR_LENGTH*sizeof(char));
        for(int i=0;i<10;i++)
        {
            sprintf(str,"%x,%d,%d",header.nw_src,header.tp_src,i);
            ...
            for(int j=0;j<3;j++)
            {
                ...
                hash_value[i]+=(int)str_out[j];
            }
            fprintf(fp, "%d,",hash_value[i]);
            fprintf(fp,"\n");     
        }
        free(str);
        fclose(fp);
            ...
            &&mgr->T[hash_value[5]]=='1'&&mgr->T[hash_value[6]]=='1'&&mgr->T[hash_value[7]]=='1'&&mgr->T[hash_value[8]]=='1'&&mgr->T[hash_value[9]]=='1'))
        {
                ...
                fp=fopen("/home/tuyen/prj/debug1.log","a");
                fprintf(fp, "notpass\n");
                fclose(fp);
                return;
        }
    ...
    fp=fopen("/home/tuyen/prj/time.log","a");
